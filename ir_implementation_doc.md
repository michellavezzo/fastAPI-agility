...
Instalacao esperada na Raspberry:

```bash
sudo apt update
sudo apt install pigpio python3-pigpio python3-rpi.gpio
...
- Confirmar na Raspberry:
  ...
  - `pigpiod` ativo.
- Confirmar que `/config/ir/status` mostra `pigpio_conectado=true` quando `AGILITY_IR_PWM_BACKEND=auto` ou `pigpio`.
